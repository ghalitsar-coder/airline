...
| 9 | **Crew Service** | Quarkus | 8009 | `crew_db` | employees, crew_members, flight_crew_assignments | OptaPlanner constraint solver (future), jam terbang compliance |
| 10 | **Maintenance Service** | Quarkus | 8010 | `maintenance_db` | maintenance_records, employees (teknisi) | Hibernate Envers audit trail, Camunda BPM approval workflow, DGCA compliance |
...
```
1. [Booking Svc]   Terima request, validasi data penumpang
2. [Inventory Svc] Lock seat via gRPC (timeout 3 detik)
3. [Pricing Svc]   Hitung total harga (base fare + ancillary)
4. [Payment Svc]   Inisiasi payment request ke Xendit
5. [Payment Svc]   Tunggu webhook callback dari Xendit (async)
6. [Payment Svc]   Payment SUCCESS → publish PaymentCompleted event
7. [Booking Svc]   Confirm booking, generate PNR
8. [Inventory Svc] Konfirmasi permanent seat assignment (release lock → assign)
9. [Loyalty Svc]   Publish event → credit miles (async)
10.[Notification]  Kirim konfirmasi booking via email (async)
```

**Saga Compensation (Rollback):**
- Step 2 gagal (seat tidak available) → return error 409, tidak perlu kompensasi
- Step 4/5 gagal (payment gagal) → release seat lock di Inventory
- Step 7 gagal → release seat lock, void payment di Payment Service
...
pricing_db      ← Pricing Service (Quarkus)
  tables: flight_prices, promotions
...
crew_db         ← Crew Service (Quarkus)
  tables: employees (crew subset), crew_members, flight_crew_assignments

maintenance_db  ← Maintenance Service (Quarkus)
  tables: maintenance_records, employees (teknisi subset),
...
- [ ] Setup ELK Stack + Prometheus/Grafana dasar
